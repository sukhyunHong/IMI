...
__attribute__ ((section(".dom1.data"))) uintptr_t thread1  = 0;
...
__attribute__ ((section(".dom1.text"))) void* pthread1(void* arg)
{
	...
	thread1 = (uintptr_t) arg;
    for (size_t i =0; i < 1000; i++){
        thread1++;
        sched_yield();//user level에서 프로세스가 다른 프로세스에 넘길때 
    }
    printf("thread1 returning %lx", thread1);
    //pthread_exit((void*)var);
    return (void*)thread1; // same as pthread_exit
}



__attribute__ ((section(".dom2.data"))) uintptr_t thread2  = 0;
...
__attribute__ ((section(".dom2.text"))) void* pthread2(void* arg)
{
    //thread2
    printf("I am thread 1 : %p\n", arg);
	thread2 = (uintptr_t) arg;
    for (size_t i =0; i < 1000; i++){
        thread2++;
        sched_yield();//user level에서 프로세스가 다른 프로세스에 넘길때 
    }
    printf("thread1 returning %lx", thread2);
    //pthread_exit((void*)var);
    return (void*)thread2; // same as pthread_exit
}
...
int main(void)
{
	int ret;
	int dom_num = 1;
	void (*fn_addr)(void) = thread1;
	uint64_t *new_ttbr0;
	unsigned long *stack_addr;

	void (*fn_addr1)(void) = thread2;
	uint64_t *new_ttbr1;
    ...
    // int ret;
    pthread_t thread1, thread2;//, thread3;
    ret = pthread_create(&thread1, NULL, pthread1, (void*)PATTERN1);
    // assert(ret == 0);
    ret = pthread_create(&thread2, NULL, pthread2, (void*)PATTERN2);
    // assert(ret == 0);
    // ret = pk_pthread_create(&thread3, NULL, pthread3, (void*)PATTERN3);
    // assert(ret == 0);
    printf("Main waiting for other thread");
