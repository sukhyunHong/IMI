...
/// Isolation module
// static __latent_entropy int iso_copy_address(struct mm_struct *mm,
// 					struct mm_struct *oldmm, unsigned long addr, 
// 					unsigned long size)
// {
// 	struct vm_area_struct *mpnt, *tmp, *prev, **pprev;
// 	struct rb_node **rb_link, *rb_parent;
...
// 	for (mpnt = oldmm->mmap; mpnt; mpnt = mpnt->vm_next) {
// 		struct file *file;
// 		printk("iso_alloc from : %lx, end: %lx\n", mpnt->vm_start, mpnt->vm_end);
...
// 		mm->map_count++;
// 		if (!(tmp->vm_flags & VM_WIPEONFORK))
// 			retval = iso_copy_page_range(mm, oldmm, mpnt, mpnt->vm_start, mpnt->vm_end-mpnt->vm_start);
...
static __latent_entropy int iso_dup_mmap(struct mm_struct *mm,
					struct mm_struct *oldmm, unsigned long stack_addr)
{
	int retval;
	LIST_HEAD(uf);
...
int make_domain_test(unsigned long *stack_addr)
{
	...
	*stack_addr = ksys_mmap_pgoff(0, stack_size + barrier_size, 
	...
	if(unlikely(*stack_addr == -1) || unlikely(*stack_addr == 0))
		goto free_pt;
	
	// change (without barrier) stack's prot.
	if(unlikely(do_mprotect_pkey(*stack_addr + barrier_size, 
					stack_size, PROT_READ | PROT_WRITE, -1) != 0)) {
		// failed to change prot.
		// So, call munmap.
		// SYSCALL_DEFINE2(munmap, unsigned long, addr, size_t, len)
		*stack_addr = untagged_addr(*stack_addr);
		profile_munmap(*stack_addr);
		iso_vm_munmap(*stack_addr, barrier_size + stack_size, true);
		goto free_pt;
	}
